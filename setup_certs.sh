...
printf "subjectAltName=IP:127.0.0.1,IP:172.17.0.1" | openssl x509 -req -extfile /dev/stdin -in certs/keycloak/keycloak.csr -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out certs/keycloak/keycloak.pem
...
openssl req -new -subj "$CERTS_DN/CN=OSDB" -key certs/os-dashboards/os-dashboards.key -out certs/os-dashboards/os-dashboards.csr
